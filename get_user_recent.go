...
// GetUserRecent - Return the recent scores from the past 24 hours for the specified user user
func (api API) GetUserRecent(options M) (*[]Score, error) {
	// Create the request object we are going to add the query too
	req, err := http.NewRequest("GET", "https://osu.ppy.sh/api/get_user_recent", nil)
	if err != nil {
		return nil, err
	}
