...
// GetUserBest - Return the top scores for the specified user user
func (api API) GetUserBest(options M) (*[]Score, error) {
	// Create the request object we are going to add the query too
	req, err := http.NewRequest("GET", "https://osu.ppy.sh/api/get_user_best", nil)
	if err != nil {
		return nil, err
	}
